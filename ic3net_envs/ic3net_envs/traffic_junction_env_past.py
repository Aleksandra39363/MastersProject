...
class TrafficJunctionEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Check if episode was already completed in previous step
        # (before we reset is_completed)
        was_completed = np.any(self.is_completed == 1) if hasattr(self, 'is_completed') else False
        was_empty = self.cars_in_sys == 0 if hasattr(self, 'cars_in_sys') else False
        ...
        # Don't add new cars if episode just ended (car completed or crashed)
        # Check completion status immediately after actions
        # A car is considered completed if is_completed flag is set (regardless of cars_in_sys)
        just_completed = np.any(self.is_completed == 1)
        # Only consider previous completion if it was completed before
        previous_completion = was_completed
        just_ended = just_completed or previous_completion or self.has_failed
        ...
        # Check if all cars have completed their routes (episode should end)
        # Check both current completion AND if it was completed in previous step
        if just_completed or previous_completion:
            self.episode_over = True
            # Add terminal reward for successful completion
            if not self.has_failed:
                rewards = rewards + self.reward_terminal()
        
        # Also end episode if car crashed
        if self.has_failed:
            self.episode_over = True
        ...
        # Only update stat at episode end (avoid double counting across steps)
        if self.episode_over:
            self.stat['success'] = 1 - self.has_failed
        self.stat['add_rate'] = self.add_rate

        terminated = self.episode_over
        truncated = False  # Set to True if episode truncated by time limit

        # Return scalar reward for gymnasium compatibility, include per-agent
        # rewards in info so trainer logic that expects arrays continues to work.
    ...
    def _get_reward(self):
        ...
        for i, l in enumerate(self.car_loc):
            if (len(np.where(np.all(self.car_loc[:i] == l,axis=1))[0]) or \
               ...
               self.has_failed = 1
               # Kill the crashed car - but only if it's still alive (prevent multiple decrements)
    ...
    def reward_terminal(self):
        ...
        terminal_reward = np.zeros(self.ncar)
        # Give +10.0 reward for each car that completed without crashing
        if not self.has_failed:
            terminal_reward = self.is_completed.astype(float) * 10.0
