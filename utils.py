...
def merge_stat(src, dest):
    # Environment-specific metrics that should be kept from the last episode only, not accumulated
    # These are per-episode and should not be summed across episodes
    skip_accumulation = {'success', 'add_rate'}
    
    for k, v in src.items():
        # Skip accumulation for environment metrics - just overwrite with latest value
        if k in skip_accumulation:
            dest[k] = v
        elif not k in dest:
            dest[k] = v
...
def init_args_for_env(parser):
    ...
    if hasattr(unwrapped_env, 'init_args'):
        unwrapped_env.init_args(parser)
